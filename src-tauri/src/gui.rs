...
impl GuiContext {
    pub fn init() -> anyhow::Result<Self> {
        ...
        let graphics_context = unsafe { GraphicsContext::new(&window, &window) }.map_err(|e| {
            anyhow::Error::msg(format!("Unable to initialize graphics context: {e}"))
        })?;
        */
        ...
        */
        ...
        event_loop.run(move |event, _, control_flow| {
            ...
            match event {
                Event::WindowEvent { window_id, event } if window_id == window.id() => {
                    match event {
                        ...
                        WindowEvent::KeyboardInput { input, .. } => {
                            let scancode = ironrdp::input::Scancode::from_u16(
                                u16::try_from(input.scancode).unwrap(),
                            );
                        ...
                        WindowEvent::ModifiersChanged(state) => {
                            const SHIFT_LEFT: ironrdp::input::Scancode =
                                ironrdp::input::Scancode::from_u8(false, 0x2A);
                            const CONTROL_LEFT: ironrdp::input::Scancode =
                                ironrdp::input::Scancode::from_u8(false, 0x1D);
                            const ALT_LEFT: ironrdp::input::Scancode =
                                ironrdp::input::Scancode::from_u8(false, 0x38);
                            const LOGO_LEFT: ironrdp::input::Scancode =
                                ironrdp::input::Scancode::from_u8(true, 0x5B);

                            let mut operations =
                                smallvec::SmallVec::<[ironrdp::input::Operation; 4]>::new();

                            let mut add_operation =
                                |pressed: bool, scancode: ironrdp::input::Scancode| {
                        ...
                        WindowEvent::MouseWheel { delta, .. } => {
                            let mut operations =
                                smallvec::SmallVec::<[ironrdp::input::Operation; 2]>::new();
                        ...
                        WindowEvent::MouseInput { state, button, .. } => {
                            let mouse_button = match button {
                                ...
                                event::MouseButton::Other(native_button) => {
                                    if let Some(button) =
                                        ironrdp::input::MouseButton::from_native_button(
                                            native_button,
                                        )
                                    {
                                        button
                ...
                Event::UserEvent(RdpOutputEvent::Image {
                    buffer,
                    width,
                    height,
                }) => {
